...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    // Ensure canvas has proper dimensions before initializing WebGL
    const initializeCanvasSize = () => {
      const pixelRatio = window.devicePixelRatio || 1;
      const width = Math.floor((canvas.clientWidth || window.innerWidth) * pixelRatio);
      const height = Math.floor((canvas.clientHeight || window.innerHeight) * pixelRatio);
    ...
    // Initialize canvas size immediately and ensure it's ready
    ...
    const pointers: Pointer[] = [Object.create(pointerPrototype.prototype) as Pointer];
    pointerPrototype.call(pointers[0]);
    ...
    function createFBO(w: number, h: number, internalFormat: number, format: number, type: number | null, param: number): FBO {
    ...
    function createDoubleFBO(w: number, h: number, internalFormat: number, format: number, type: number | null, param: number): DoubleFBO {
    ...
    function resizeFBO(target: FBO, w: number, h: number, internalFormat: number, format: number, type: number | null, param: number): FBO {
    ...
    function resizeDoubleFBO(target: DoubleFBO, w: number, h: number, internalFormat: number, format: number, type: number | null, param: number): DoubleFBO {
